...
static struct data_control_source *source_from_resource(
	...
	assert(wl_resource_instance_of(resource,
		&ext_data_control_source_v1_interface, &source_impl));
...
static void data_offer_destroy(struct data_offer *offer) {
	...
	struct wlr_ext_data_control_device_v1 *device = offer->device;
	if (device != NULL) {
		if (offer->is_primary) {
...
static struct data_offer *data_offer_from_offer_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&ext_data_control_offer_v1_interface, &offer_impl));
	return wl_resource_get_user_data(resource);
}
...
static struct wl_resource *create_offer(struct wlr_ext_data_control_device_v1 *device,
	...
	struct wl_resource *resource = wl_resource_create(client,
		&ext_data_control_offer_v1_interface, version, 0);
	if (resource == NULL) {
		free(offer);
		return NULL;
	}
	...
	ext_data_control_device_v1_send_data_offer(device->resource, resource);

	char **p;
	wl_array_for_each(p, mime_types) {
		ext_data_control_offer_v1_send_offer(resource, *p);
	}
...
static const struct ext_data_control_device_v1_interface control_impl;

static struct wlr_ext_data_control_device_v1 *control_from_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&ext_data_control_device_v1_interface, &control_impl));
	return wl_resource_get_user_data(resource);
}
...
static void control_send_selection(struct wlr_ext_data_control_device_v1 *device) {
	...
	ext_data_control_device_v1_send_selection(device->resource,
		device->selection_offer_resource);
}
...
static void control_send_primary_selection(
		struct wlr_ext_data_control_device_v1 *device) {
	uint32_t version = wl_resource_get_version(device->resource);
	if (version < EXT_DATA_CONTROL_DEVICE_V1_PRIMARY_SELECTION_SINCE_VERSION) {
		return;
	}
	...
	ext_data_control_device_v1_send_primary_selection(device->resource,
		device->primary_selection_offer_resource);
}

static void control_handle_resource_destroy(struct wl_resource *resource) {
	struct wlr_ext_data_control_device_v1 *device = control_from_resource(resource);
	wlr_ext_data_control_device_v1_destroy(device);
}
...
void wlr_ext_data_control_device_v1_destroy(struct wlr_ext_data_control_device_v1 *device) {
	if (device == NULL) {
		return;
	}
	ext_data_control_device_v1_send_finished(device->resource);
...
static struct wlr_ext_data_control_manager_v1 *manager_from_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&ext_data_control_manager_v1_interface, &manager_impl));
	return wl_resource_get_user_data(resource);
}
...
static void manager_handle_create_data_source(struct wl_client *client,
	...
	source->resource = wl_resource_create(client,
		&ext_data_control_source_v1_interface, version, id);
...
static void manager_handle_get_data_device(struct wl_client *client,
	...
	uint32_t version = wl_resource_get_version(manager_resource);
	struct wl_resource *resource = wl_resource_create(client,
		&ext_data_control_device_v1_interface, version, id);
	if (resource == NULL) {
		wl_resource_post_no_memory(manager_resource);
		return;
	}
	...
	struct wlr_ext_data_control_device_v1 *device = calloc(1, sizeof(*device));
	if (device == NULL) {
		wl_resource_post_no_memory(manager_resource);
		return;
	}
...
static void manager_bind(struct wl_client *client, void *data, uint32_t version,
		uint32_t id) {
	struct wlr_ext_data_control_manager_v1 *manager = data;
...
struct wlr_ext_data_control_manager_v1 *wlr_ext_data_control_manager_v1_create(
	...
	assert(version <= EXT_DATA_CONTROL_MANAGER_VERSION);

	struct wlr_ext_data_control_manager_v1 *manager = calloc(1, sizeof(*manager));
	if (manager == NULL) {
		return NULL;
	}
