		...
		if (scene_buffer->dst_width > 0 && scene_buffer->dst_height > 0) {
			...
			*height = scene_buffer->dst_height;
			...
			if (scene_buffer->transform & WL_OUTPUT_TRANSFORM_90) {
				*height = scene_buffer->raster->width;
				*width = scene_buffer->raster->height;
			} else {
...
static bool scene_entry_try_direct_scanout(struct render_list_entry *entry,
	...
	if (buffer->transform & WL_OUTPUT_TRANSFORM_90) {
		default_box.width = buffer->raster->height;
		default_box.height = buffer->raster->width;
	} else {
		default_box.width = buffer->raster->width;
		default_box.height = buffer->raster->height;
	}
