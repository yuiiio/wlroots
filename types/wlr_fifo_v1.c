...
static int handle_timer(void *data) {
	...
	if (fifo->barrier_set && (now_nsec - (int64_t)fifo->last_output_present_nsec > refresh_nsec)) {
		fifo_signal_barrier(fifo);
	}

	// we only need to re-arm the timer if there are pending commits in the queue
	if (!wl_list_empty(&fifo->commits)) {
		wl_event_source_timer_update(fifo->surface_occluded_source, 100);
	}
	else {
		fifo->surface_occluded_source_armed = false;
	}
...
static void fifo_manager_handle_get_fifo(struct wl_client *wl_client, struct wl_resource *manager_resource,
	...
	// a heuristically defined value (10hz) so that we can advance the queue.
	//
	// The timer is armed only when commits are actually queued, and possibly re-armed when it
	// expires.
